...
Выполняется SQL-запрос, который подсчитывает общее число записей в таблице products с помощью функции COUNT(*).

1. Определение количества записей на странице и общего числа страниц
...
Если записей нет, переменная $totalPages установится в 1 (чтобы избежать деления на ноль).
...
```
Если параметр отсутствует или не является числом, по умолчанию используется 1.
Вторая строка кода гарантирует что номер страницы не меньше 1 и в пределах до `$totalPages`

3. Смещение определяет, сколько записей нужно пропустить при выборке. Вычисление происходит по формуле:
```php
...
Описание механизма:
 ...
 - LIMIT задаёт количество записей, выводимых на странице.
...
4. Выборка записей с использованием LIMIT и OFFSET:
...
```
 ...
 - Запрос выбирает все столбцы из таблицы products, сортируя записи по полю created_at в порядке убывания
 - `LIMIT :limit` указывает, сколько записей необходимо вернуть
 - `OFFSET :offset` указывает, сколько записей нужно пропустить
 - Метод `bindValue()` привязывает числовые значения к заполнителям `:limit` и `:offset` с типом `PDO::PARAM_INT`, что защищает запрос от SQL-инъекций и обеспечивает корректную передачу значений.
 - метод `fetchAll(PDO::FETCH_ASSOC)` возвращает массив записей
